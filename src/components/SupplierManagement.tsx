...
export default function SupplierManagement({
  ...
  return (
    <div className="space-y-6" id="supplier-management-page">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-4">
          ...
          <div className="flex items-center gap-2">
            <span className="text-[11px] font-mono text-slate-600 bg-white border border-slate-200 px-3 py-1.5 rounded-xl shadow-xs">
              ทั้งหมด <b className="text-slate-900">{totalSuppliersCount}</b> ราย | ยอดซื้อรวม <b className="text-emerald-700 font-mono">฿{totalSpendAll.toLocaleString('th-TH', { minimumFractionDigits: 2 })}</b>
            </span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mt-4">
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-4 shadow-xs">
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="text-[10px] text-emerald-700 font-bold">PO Value</span>
            </div>
            <div className="h-44 flex items-center justify-center">
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-4 shadow-xs flex flex-col justify-between">
                  ...
                  topOutstandingSuppliers.map((sup, idx) => (
                    <div key={idx} className="flex items-center justify-between text-xs py-1 border-b border-slate-100 last:border-0">
                      <div className="flex items-center gap-2 truncate pr-2">
                        ...
                        <span className="text-slate-800 truncate font-medium">{sup.supplier_name}</span>
                      </div>
            ...
            {topSuppliersBySpend.length > 0 && (
              <div className="mt-3 pt-2.5 border-t border-slate-100 flex items-center justify-between text-[11px] bg-slate-50 p-2 rounded-xl">
                <span className="text-slate-600 truncate">
                  ซื้อสูงสุด: <b className="text-slate-900">{topSuppliersBySpend[0].supplier_name}</b>
                </span>
                <span className="font-mono text-emerald-700 font-bold shrink-0">฿{topSuppliersBySpend[0].totalPurchases.toLocaleString()}</span>
              </div>
            )}
          </div>
        </div>
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-4 flex flex-wrap items-center justify-between gap-3 shadow-xs">
        <div className="flex items-center gap-2">
          ...
          {selectedIds.length > 0 && (
            <span className="text-xs bg-indigo-50 text-indigo-700 border border-indigo-200 px-3 py-1.5 rounded-xl font-medium">
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-4 shadow-xs space-y-3">
        <div className="flex items-center justify-between border-b border-slate-100 pb-2.5">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-bold text-slate-900 uppercase tracking-wider">ค้นหาและตัวกรองข้อมูลคู่ค้า (Filter Panel)</span>
          </div>
          <button
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 text-xs">
          ...
          <div className="lg:col-span-2">
            <label className="text-[10px] text-slate-600 font-semibold mb-1 block">ค้นหา (รหัส, ชื่อ, Tax ID, ผู้ติดต่อ, เบอร์, อีเมล)</label>
            <div className="relative">
              ...
              <input
                ...
                className="w-full bg-white border border-slate-300 rounded-xl pl-8 pr-3 py-1.5 text-xs text-slate-800 placeholder-slate-400 focus:outline-none focus:border-indigo-500"
              />
            </div>
          </div>
          ...
          {/* Supplier Type */}
          <div>
            <label className="text-[10px] text-slate-600 font-semibold mb-1 block">Supplier Type</label>
            <select
              value={filterSupplierType}
              onChange={e => setFilterSupplierType(e.target.value)}
              className="w-full bg-white border border-slate-300 rounded-xl px-3 py-1.5 text-xs text-slate-800 focus:outline-none focus:border-indigo-500 cursor-pointer"
            >
          ...
          {/* Payment Term */}
          <div>
            <label className="text-[10px] text-slate-600 font-semibold mb-1 block">Payment Term</label>
            <select
          ...
          {/* Status */}
          <div>
            <label className="text-[10px] text-slate-600 font-semibold mb-1 block">Status</label>
            <select
              value={filterStatus}
          ...
          {/* Created Date Range */}
          <div>
            <label className="text-[10px] text-slate-600 font-semibold mb-1 block">วันที่สร้าง From – To</label>
            <div className="flex items-center gap-1">
              <input
                type="date"
                value={filterDateFrom}
                onChange={e => setFilterDateFrom(e.target.value)}
                className="w-1/2 bg-white border border-slate-300 rounded-xl px-2 py-1.5 text-[10px] text-slate-800 focus:outline-none focus:border-indigo-500"
              />
      ...
      {/* ============================================================ */}
      {/* SECTION 4: SUPPLIER LIST TABLE OR TEMPLATE VIEW               */}
      {/* ============================================================ */}
      {viewMode === 'template' ? (
        <div className="space-y-4">
          <div className="bg-white border border-slate-200 rounded-2xl p-4 shadow-xs flex flex-wrap items-center justify-between gap-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-sm font-bold text-slate-900">มุมมองเอกสารเทมเพลตประวัติคู่ค้า (A4 Document Template View)</h3>
                <p className="text-xs text-slate-500">เลือกคู่ค้าเพื่อแสดงเอกสารทางการ พร้อมพิมพ์หรือบันทึกเป็น PDF</p>
              </div>
            </div>

            <div className="flex items-center gap-2">
              <label className="text-xs font-semibold text-slate-600">เลือกคู่ค้า:</label>
              <select
                value={selectedTemplateSupplier?.id || ''}
                onChange={(e) => {
                  const sup = filteredList.find(s => s.id === e.target.value);
                  if (sup) setSelectedTemplateSupplier(sup);
                }}
                className="bg-white border border-slate-300 rounded-xl px-3 py-1.5 text-xs text-slate-800 font-semibold focus:outline-none focus:border-indigo-500 cursor-pointer shadow-xs"
              >
          ...
          {selectedTemplateSupplier ? (
            <div className="bg-white border border-slate-200 rounded-2xl p-4 sm:p-6 shadow-xs">
              <SupplierProfileTemplate
                supplier={selectedTemplateSupplier}
        ...
        <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-xs">
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between border-b border-slate-200 bg-slate-50 divide-y sm:divide-y-0 sm:divide-x divide-slate-200">
            <div className="flex items-center px-4 py-2 flex-grow min-w-0">
              ...
              <div className="font-mono text-[11px] text-slate-700 bg-white border border-slate-200 py-1 px-2.5 rounded flex-1 truncate select-all">
                =FILTER(SUPPLIER_DATABASE, SEARCH(&quot;{searchTerm || '*'}&quot;, SupplierName) * Type=&quot;{filterSupplierType}&quot; * Status=&quot;{filterStatus}&quot;)
              </div>
            </div>
            <div className="flex items-center gap-2 px-3 py-1.5 bg-slate-50 text-xs text-slate-600">
              <span className="font-medium bg-white px-2 py-0.5 rounded border border-slate-200 text-slate-700 text-[11px] select-none">Sheet1 (Suppliers)</span>
              <span className="text-slate-300">|</span>
              <span className="text-indigo-600 font-mono text-[11px] font-bold">{filteredList.length} Records</span>
            </div>
          </div>

          <div className="p-3.5 bg-slate-50/50 border-b border-slate-200 flex flex-wrap items-center justify-between gap-2">
            <div className="flex items-center gap-2">
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="py-3.5 px-3 max-w-[200px]">
                          <div className="font-bold text-slate-900 truncate">{sup.supplier_name}</div>
                        ...
                        {/* Contact Person */}
                        <td className="py-3.5 px-3 text-slate-700 truncate max-w-[130px]">
                          {sup.contact_person || '-'}
                        </td>
                        ...
                        {/* Email */}
                        <td className="py-3.5 px-3 font-mono text-slate-500 truncate max-w-[140px]" title={sup.email}>
                          {sup.email || '-'}
                        </td>
                        ...
                        <td className="py-3.5 px-3 text-right font-mono font-bold whitespace-nowrap">
                          <span className={sup.outstanding > 0 ? 'text-rose-600' : 'text-slate-500'}>
                        ...
                        <td className="py-3.5 px-3 text-center whitespace-nowrap">
                          <div className="flex items-center justify-center gap-1">
                            ...
                            <button
                              ...
                              className="flex items-center gap-1 px-2 py-1 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 border border-indigo-200 rounded-lg text-[10.5px] font-medium transition cursor-pointer"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-slate-900/40 backdrop-blur-xs overflow-y-auto">
          <div className="bg-white border border-slate-200 rounded-2xl w-full max-w-5xl max-h-[92vh] flex flex-col shadow-2xl overflow-hidden my-auto">
            ...
            <div className="p-4 bg-slate-50 border-b border-slate-200 flex flex-wrap items-center justify-between gap-3">
              <div className="flex items-center gap-3">
                <span className="p-2.5 bg-indigo-50 text-indigo-700 border border-indigo-200 rounded-xl">
                  <Building2 className="w-5 h-5" />
                </span>
                <div>
                  <div className="flex items-center gap-2">
                    <h3 className="text-base font-bold text-slate-900">{viewingSupplier.supplier_name}</h3>
                    <span className="font-mono text-xs text-indigo-700 font-bold bg-indigo-50 border border-indigo-200 px-2 py-0.5 rounded-lg">
            ...
            {/* Navigation Tabs */}
            <div className="flex overflow-x-auto border-b border-slate-200 bg-slate-100/70 px-3 py-1.5 gap-1 text-xs">
              {[
            ...
            <div className="p-5 overflow-y-auto space-y-4 text-xs text-slate-700">
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                    <div className="bg-slate-50 border border-slate-200 p-4 rounded-xl">
                      <div className="text-[10px] text-slate-500 uppercase font-bold">ยอดจัดซื้อสะสมทั้งหมด</div>
                      <div className="text-xl font-bold font-mono text-emerald-600 mt-1">
                        ฿{((viewingSupplier as any).totalPurchases || 0).toLocaleString('th-TH', { minimumFractionDigits: 2 })}
                      </div>
                      <div className="text-[10px] text-slate-400 mt-1">จากใบสั่งซื้อทั้งหมด</div>
                    </div>
                    <div className="bg-slate-50 border border-slate-200 p-4 rounded-xl">
                      <div className="text-[10px] text-slate-500 uppercase font-bold">ยอดเจ้าหนี้ค้างชำระ</div>
                      ...
                      <div className="text-[10px] text-slate-400 mt-1">วงเงินเครดิต: ฿{(viewingSupplier.credit_limit || 500000).toLocaleString()}</div>
                    </div>
                    <div className="bg-slate-50 border border-slate-200 p-4 rounded-xl">
                      <div className="text-[10px] text-slate-500 uppercase font-bold">เงื่อนไขการค้า (Payment Term)</div>
                      <div className="text-lg font-bold text-slate-900 mt-1">
                        {viewingSupplier.payment_term || 'Credit 30'}
                      </div>
                      <div className="text-[10px] text-slate-400 mt-1">การซื้อล่าสุด: {(viewingSupplier as any).lastPurchase || '-'}</div>
                    </div>
                  </div>

                  <div className="border border-slate-200 rounded-xl p-4 bg-slate-50/50 space-y-3">
                    <h4 className="text-xs font-bold text-slate-900 uppercase tracking-wider">ข้อมูลทั่วไปของสถานประกอบการ</h4>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        <div className="text-[10px] text-slate-500">ชื่อสถานประกอบการ (ทางการ)</div>
                        <div className="font-bold text-slate-900 text-sm">{viewingSupplier.supplier_name}</div>
                        <div className="text-[10px] text-slate-500 mt-2">เลขประจำตัวผู้เสียภาษี 13 หลัก</div>
                        <div className="font-mono text-slate-900 font-bold">{viewingSupplier.tax_id || '-'}</div>
                        <div className="text-[10px] text-slate-500 mt-2">ประเภทสถานประกอบการ</div>
                        <div className="text-slate-800">{viewingSupplier.supplier_type || 'Distributor'}</div>
                      </div>
                      <div>
                        <div className="text-[10px] text-slate-500">ผู้ติดต่อหลัก (Primary Contact)</div>
                        <div className="text-slate-900 font-bold">{viewingSupplier.contact_person || '-'}</div>
                        <div className="text-[10px] text-slate-500 mt-2">เบอร์โทรศัพท์ติดต่อ</div>
                        <div className="font-mono text-slate-900">{viewingSupplier.phone || '-'}</div>
                        <div className="text-[10px] text-slate-500 mt-2">อีเมลติดต่อ</div>
                        <div className="font-mono text-indigo-600">{viewingSupplier.email || '-'}</div>
                      </div>
                    </div>
                    {viewingSupplier.notes && (
                      <div className="pt-2 border-t border-slate-200 mt-2">
                        <div className="text-[10px] text-slate-500 mb-0.5">บันทึกเพิ่มเติม / สินค้าเฉพาะทาง:</div>
                        <div className="text-slate-700 italic">{viewingSupplier.notes}</div>
                      </div>
                    )}
                  </div>
                </div>
              ...
              {detailTab === 'contact' && (
                <div className="space-y-4">
                  <div className="flex justify-between items-center">
                    <div>
                      <h4 className="text-xs font-bold text-slate-900">รายชื่อผู้ติดต่อประจำคู่ค้า (Supplier Representatives)</h4>
                      <p className="text-[11px] text-slate-500 mt-0.5">จัดการข้อมูลผู้ติดต่อ ฝ่ายขาย บัญชี หรือประสานงาน</p>
                    </div>
                    <button
                  ...
                  {viewingSupplier.contacts && viewingSupplier.contacts.length > 0 ? (
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      {viewingSupplier.contacts.map((contact, idx) => (
                        <div key={contact.id || idx} className="bg-slate-50 p-4 rounded-xl border border-slate-200 hover:border-slate-300 transition-all text-xs space-y-2 relative group">
                          <div className="flex justify-between items-start">
                            <div>
                              ...
                              <div className="text-[11px] text-slate-500 mt-0.5 pl-5">
                                ตำแหน่ง: <span className="text-slate-700 font-medium">{contact.role || '-'}</span>
                              </div>
                            </div>
                            <div className="flex items-center gap-1">
                              ...
                              <button
                                ...
                                className="p-1 text-slate-400 hover:text-amber-600 hover:bg-slate-100 rounded border border-slate-200 cursor-pointer"
                              >
                              ...
                              <button
                                ...
                                title="ลบผู้ติดต่อ"
                                className="p-1 text-slate-400 hover:text-rose-600 hover:bg-slate-100 rounded border border-slate-200 cursor-pointer"
                              >
                          ...
                          <div className="grid grid-cols-1 gap-1 pt-2 border-t border-slate-200 font-mono text-[11px]">
                    ...
                    <div className="text-center py-10 bg-slate-50 border border-dashed border-slate-300 rounded-xl space-y-2">
                      ...
                      <p className="text-xs text-slate-500">ยังไม่มีรายชื่อผู้ติดต่อสำหรับคู่ค้ารายนี้</p>
                      <button
                        onClick={() => handleOpenContactForm()}
                        className="text-xs font-semibold text-indigo-600 hover:underline inline-block focus:outline-none cursor-pointer"
                      >
              ...
              {detailTab === 'address' && (
                <div className="space-y-3">
                  <h4 className="text-xs font-bold text-slate-900">สถานที่ตั้งและที่อยู่สำหรับออกเอกสาร (Addresses)</h4>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              ...
              {detailTab === 'financial' && (
                <div className="space-y-4">
                  <h4 className="text-xs font-bold text-slate-900">ข้อมูลทางการเงินและบัญชีธนาคาร (Financial & Bank Account)</h4>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 space-y-2">
                      <div className="text-[11px] text-slate-600 font-bold">เงื่อนไขเครดิตทางการค้า</div>
                      <div className="flex justify-between py-1 border-b border-slate-200">
                        <span className="text-slate-500">Credit Term:</span>
                        <span className="font-bold text-slate-900">{viewingSupplier.payment_term || 'Credit 30'}</span>
                      </div>
                      <div className="flex justify-between py-1 border-b border-slate-200">
                        <span className="text-slate-500">Credit Limit:</span>
                        <span className="font-mono font-bold text-emerald-600">฿{(viewingSupplier.credit_limit || 500000).toLocaleString()}</span>
                      </div>
                      <div className="flex justify-between py-1 border-b border-slate-200">
                        <span className="text-slate-500">ยอดค้างชำระปัจจุบัน:</span>
                        <span className="font-mono font-bold text-rose-600">฿{((viewingSupplier as any).outstanding || 0).toLocaleString()}</span>
                      </div>
                      <div className="flex justify-between py-1">
                        <span className="text-slate-500">วงเงินคงเหลือ (Available):</span>
                    ...
                    <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 space-y-2">
                      <div className="text-[11px] text-slate-600 font-bold">ข้อมูลบัญชีธนาคารสำหรับจ่ายเงิน (Bank Account)</div>
                      <div className="flex justify-between py-1 border-b border-slate-200">
                        <span className="text-slate-500">ธนาคาร:</span>
                        <span className="font-bold text-slate-900">{viewingSupplier.bank_account?.bank_name || 'ธนาคารกสิกรไทย (KBANK)'}</span>
                      </div>
                      <div className="flex justify-between py-1 border-b border-slate-200">
                        <span className="text-slate-500">เลขที่บัญชี:</span>
                        <span className="font-mono font-bold text-emerald-600">{viewingSupplier.bank_account?.account_number || '741-2-19882-9'}</span>
                      </div>
                      <div className="flex justify-between py-1">
                        <span className="text-slate-500">ชื่อบัญชี:</span>
                        <span className="text-slate-800">{viewingSupplier.bank_account?.account_name || viewingSupplier.supplier_name}</span>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              {detailTab === 'pr' && (
                <div className="space-y-3">
                  <div className="flex justify-between items-center">
                    <h4 className="text-xs font-bold text-slate-900">ประวัติใบขอซื้อ (Purchase Requisition - PR)</h4>
                    <span className="text-[11px] text-slate-500">{(viewingSupplier as any).linkedPRs?.length || 0} รายการ</span>
                  </div>
                    ...
                    <table className="w-full text-left border-collapse text-xs">
                      ...
                      <tbody className="divide-y divide-slate-100">
                        {((viewingSupplier as any).linkedPRs || []).length === 0 ? (
                          <tr><td colSpan={5} className="text-center py-6 text-slate-400">ไม่มีประวัติ PR</td></tr>
                        ) : (
                          (viewingSupplier as any).linkedPRs.map((pr: any) => (
                            <tr key={pr.id}>
                              <td className="py-2.5 px-3 font-mono font-bold text-cyan-600">{pr.pr_no}</td>
                              <td className="py-2.5 px-3 text-slate-600 font-mono">{pr.date || pr.required_date || '-'}</td>
                              <td className="py-2.5 px-3 text-slate-900">{pr.requested_by || pr.requestor || '-'}</td>
              ...
              {detailTab === 'po' && (
                <div className="space-y-3">
                  <div className="flex justify-between items-center">
                    <h4 className="text-xs font-bold text-slate-900">ประวัติใบสั่งซื้อ (Purchase Order - PO)</h4>
                    <span className="text-[11px] text-slate-500">{(viewingSupplier as any).linkedOrders?.length || 0} รายการ</span>
                  </div>
                    ...
                    <table className="w-full text-left border-collapse text-xs">
                      ...
                      <tbody className="divide-y divide-slate-100">
                        {((viewingSupplier as any).linkedOrders || []).length === 0 ? (
                          <tr><td colSpan={5} className="text-center py-6 text-slate-400">ไม่มีประวัติ PO</td></tr>
                        ) : (
                          (viewingSupplier as any).linkedOrders.map((po: any) => (
                            <tr key={po.id}>
                              <td className="py-2.5 px-3 font-mono font-bold text-indigo-600">{po.po_no}</td>
                              <td className="py-2.5 px-3 text-slate-600 font-mono">{po.date || po.po_date || '-'}</td>
                              <td className="py-2.5 px-3 font-mono text-cyan-600">{po.pr_no || '-'}</td>
              ...
              {detailTab === 'purchase_history' && (
                <div className="space-y-3">
                  <h4 className="text-xs font-bold text-slate-900">ประวัติรายการสินค้าที่เคยสั่งซื้อ (Purchased Items Log)</h4>
                  <div className="space-y-2">
                    {((viewingSupplier as any).linkedOrders || []).flatMap((po: any) => po.items || []).map((it: any, idx: number) => (
                      <div key={idx} className="flex justify-between items-center bg-slate-50 p-3 rounded-xl border border-slate-200">
                        ...
                        <span className="font-mono font-bold text-emerald-600 text-xs">฿{it.amount?.toLocaleString()}</span>
                      </div>
                    ))}
                    {((viewingSupplier as any).linkedOrders || []).flatMap((po: any) => po.items || []).length === 0 && (
                      <div className="text-center py-6 text-slate-400">ยังไม่มีประวัติการซื้อสินค้า</div>
                    )}
                  </div>
                </div>
              )}
              ...
              {detailTab === 'documents' && (
                <div className="space-y-3">
                  <div className="flex justify-between items-center">
                    <h4 className="text-xs font-bold text-slate-900">เอกสารประกอบคู่ค้า (Registered Documents)</h4>
                    <button
              ...
              {detailTab === 'activity' && (
                <div className="space-y-3">
                  <h4 className="text-xs font-bold text-slate-900">ประวัติการแก้ไขและกิจกรรม (Audit Trail & Activity Log)</h4>
                  <div className="space-y-2">
                    {(viewingSupplier.activity_logs || [
                      ...
                      <div key={idx} className="flex items-start gap-2.5 bg-slate-50 p-3 rounded-xl border border-slate-200">
                        ...
                        <div className="flex-1">
                          <div className="text-slate-900 font-medium text-xs">{log.action}</div>
          ...
          <div className="bg-white border border-slate-200 rounded-2xl w-full max-w-3xl max-h-[92vh] flex flex-col shadow-2xl overflow-hidden my-auto">
            ...
            <form onSubmit={handleSubmit} className="p-5 overflow-y-auto space-y-4 text-xs">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div>
                  <label className="text-[10px] text-slate-600 font-semibold mb-1 block">รหัสผู้ขาย (Supplier Code)</label>
                  <input
                    type="text"
                    required
                    value={formCode}
                    onChange={e => setFormCode(e.target.value)}
                    className="w-full bg-white border border-slate-300 rounded-xl px-3 py-2 text-xs font-mono text-indigo-600 font-bold focus:outline-none focus:border-indigo-500"
                  />
                </div>
                <div className="sm:col-span-2">
                  <label className="text-[10px] text-slate-600 font-semibold mb-1 block">ชื่อบริษัท / Supplier Name *</label>
                  <input
                    type="text"
                    required
                    value={formName}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div>
                  <label className="text-[10px] text-slate-600 font-semibold mb-1 block">เลขประจำตัวผู้เสียภาษี (Tax ID)</label>
                  <input
                    type="text"
                    value={formTaxId}
                ...
                <div>
                  <label className="text-[10px] text-slate-600 font-semibold mb-1 block">ประเภทผู้ขาย (Supplier Type)</label>
                  <select
                    value={formSupplierType}
                    onChange={e => setFormSupplierType(e.target.value as any)}
                    className="w-full bg-white border border-slate-300 rounded-xl px-3 py-2 text-xs text-slate-800 focus:outline-none focus:border-indigo-500 cursor-pointer"
                  >
                    <option value="Distributor">Distributor (ตัวแทนจำหน่าย)</option>
                    <option value="Manufacturer">Manufacturer (ผู้ผลิต)</option>
                    <option value="Wholesaler">Wholesaler (ค้าส่ง)</option>
                ...
                <div>
                  <label className="text-[10px] text-slate-600 font-semibold mb-1 block">สถานะคู่ค้า (Status)</label>
                  <select
                    value={formStatus}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div>
                  <label className="text-[10px] text-slate-600 font-semibold mb-1 block">ผู้ติดต่อหลัก (Contact Person)</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="text-[10px] text-slate-600 font-semibold mb-1 block">เบอร์โทรศัพท์ (Phone)</label>
                  <input
                    type="text"
                    value={formPhone}
                    onChange={e => setFormPhone(e.target.value)}
                    className="w-full bg-white border border-slate-300 rounded-xl px-3 py-2 text-xs font-mono text-slate-800 focus:outline-none focus:border-indigo-500"
                  />
                </div>
                <div>
                  <label className="text-[10px] text-slate-600 font-semibold mb-1 block">อีเมล (Email)</label>
                  <input
                    type="email"
                    value={formEmail}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  <label className="text-[10px] text-slate-600 font-semibold mb-1 block">เงื่อนไขชำระเงิน (Payment Term)</label>
                  <select
                ...
                <div>
                  <label className="text-[10px] text-slate-600 font-semibold mb-1 block">วงเงินเครดิต (Credit Limit - บาท)</label>
                  <input
                    type="number"
                    value={formCreditLimit}
                    onChange={e => setFormCreditLimit(Number(e.target.value))}
                    className="w-full bg-white border border-slate-300 rounded-xl px-3 py-2 text-xs font-mono font-bold text-emerald-600 focus:outline-none focus:border-indigo-500"
                  />
                </div>
              </div>
              ...
              <div className="border border-slate-200 rounded-xl p-3 bg-slate-50 space-y-2">
                <span className="text-[11px] font-bold text-slate-900">ข้อมูลบัญชีธนาคารสำหรับโอนจ่ายเงิน</span>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                  ...
                  <div>
                    <input
                      ...
                      className="w-full bg-white border border-slate-300 rounded-lg px-2.5 py-1.5 text-xs font-mono text-emerald-600 font-bold"
                    />
                  </div>
                  <div>
                    <input
              ...
              {/* Address */}
              <div>
                <label className="text-[10px] text-slate-600 font-semibold mb-1 block">ที่อยู่สำนักงานใหญ่</label>
                <textarea
                  rows={2}
                  value={formAddress}
                  onChange={e => setFormAddress(e.target.value)}
                  className="w-full bg-white border border-slate-300 rounded-xl p-2.5 text-xs text-slate-800 focus:outline-none focus:border-indigo-500"
                />
              </div>

              {/* Notes */}
              <div>
                <label className="text-[10px] text-slate-600 font-semibold mb-1 block">บันทึกเพิ่มเติม / หมายเหตุ</label>
                <input
                  type="text"
                  value={formNotes}
            ...
            <form onSubmit={handleSaveContact} className="p-5 space-y-4">
              <div className="space-y-1 text-xs">
                <label className="font-semibold text-slate-700 block">
                  ชื่อ-นามสกุล ผู้ติดต่อ <span className="text-rose-600">*</span>
                </label>
                <input
                  ...
                  className="w-full p-2.5 bg-white border border-slate-300 rounded-xl text-slate-800 focus:outline-none focus:border-indigo-500 text-xs"
                />
              </div>

              <div className="space-y-1 text-xs">
                <label className="font-semibold text-slate-700 block">ตำแหน่ง / หน้าที่</label>
                <input
              ...
              <div className="space-y-1 text-xs">
                ...
                <input
                  ...
                  className="w-full p-2.5 bg-white border border-slate-300 rounded-xl text-slate-800 font-mono focus:outline-none focus:border-indigo-500 text-xs"
                />
              </div>
              ...
              <div className="flex justify-end gap-2.5 pt-4 border-t border-slate-200">
                <button
                  type="button"
                  onClick={() => setIsContactFormOpen(false)}
                  className="px-4 py-2 text-xs text-slate-700 bg-slate-100 hover:bg-slate-200 rounded-xl transition cursor-pointer font-medium"
                >
                  ยกเลิก
                </button>
                <button
