        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-4">
          ...
          <div className="flex items-center gap-2">
            <span className="text-[11px] font-mono text-slate-400 bg-slate-900 border border-slate-800 px-3 py-1.5 rounded-xl">
              ทั้งหมด <span className="font-bold text-white">{totalCount}</span> ฉบับ | ค้างชำระ <span className="font-bold text-amber-400">฿{totalOutstanding.toLocaleString('th-TH', { minimumFractionDigits: 2 })}</span>
            </span>
          </div>
        </div>
                  ...
                  topOutstandingCustomers.map((cust, idx) => (
                    <div key={idx} className="flex items-center justify-between text-xs py-1 border-b border-slate-800/60 last:border-0">
                      <div className="flex items-center gap-2 truncate pr-2">
                        ...
                        <span className="text-slate-300 truncate font-medium">{cust.name}</span>
                      </div>
            ...
            {upcomingDueNotes.length > 0 && (
              <div className="mt-3 pt-2.5 border-t border-slate-800/80 flex items-center justify-between text-[11px] bg-slate-950/40 p-2 rounded-xl">
                <span className="text-slate-400 flex items-center gap-1">
                  ...
                  ใกล้ครบกำหนด: <b className="text-white">{upcomingDueNotes[0].billing_no}</b> ({upcomingDueNotes[0].dueDate})
                </span>
                <span className="font-mono text-amber-400 font-bold">฿{upcomingDueNotes[0].outstanding_amount.toLocaleString()}</span>
              </div>
            )}
          </div>
        </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 flex flex-wrap items-center justify-between gap-3 shadow-sm">
        <div className="flex flex-wrap items-center gap-2">
          ...
          {selectedIds.length > 0 && (
            <span className="text-xs bg-emerald-950 text-emerald-300 border border-emerald-800 px-3 py-1.5 rounded-xl font-medium">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-2 bg-emerald-950/70 hover:bg-emerald-900/80 text-emerald-300 rounded-xl text-xs font-semibold border border-emerald-800 transition cursor-pointer"
      ...
      {viewMode === 'template' ? (
        <div className="space-y-4">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 flex flex-wrap items-center justify-between gap-3 shadow-sm">
            <div className="flex flex-wrap items-center gap-3">
              ...
              <select
                ...
                className="bg-slate-950 border border-slate-700 text-white rounded-xl px-3 py-1.5 text-xs font-mono focus:outline-none focus:border-emerald-500 cursor-pointer max-w-md"
              >
                {filteredList.map(b => (
            ...
            <button
              onClick={() => setViewMode('table')}
              className="text-xs font-semibold text-slate-300 hover:text-white px-3.5 py-1.5 bg-slate-800 hover:bg-slate-700 rounded-xl transition cursor-pointer border border-slate-700 flex items-center gap-1.5"
            >
          ...
          {filteredList.length > 0 ? (
            ...
            <div className="text-center py-16 text-slate-500 bg-slate-900 border border-slate-800 rounded-2xl text-xs">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 shadow-sm space-y-3">
        <div className="flex items-center justify-between border-b border-slate-800 pb-2.5">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-bold text-white uppercase tracking-wider">ตัวกรองเอกสาร (Advanced Filters)</span>
          </div>
          <button
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3 text-xs">
          {/* Search Box */}
          <div>
            <label className="text-[10px] text-slate-400 font-semibold mb-1 block">เลขที่ใบวางบิล / คำค้น</label>
          ...
          {/* Customer */}
          <div>
            <label className="text-[10px] text-slate-400 font-semibold mb-1 block">Customer (ลูกค้า)</label>
            <select
              value={filterCustomer}
              onChange={e => setFilterCustomer(e.target.value)}
              className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-1.5 text-xs text-white focus:outline-none focus:border-emerald-500 cursor-pointer"
            >
          ...
          {/* Status */}
          <div>
            <label className="text-[10px] text-slate-400 font-semibold mb-1 block">Status (สถานะ)</label>
            <select
              value={filterStatus}
          ...
          {/* Salesperson */}
          <div>
            <label className="text-[10px] text-slate-400 font-semibold mb-1 block">Salesperson / ผู้ส่งมอบ</label>
            <select
          ...
          {/* Billing Date Range */}
          <div>
            <label className="text-[10px] text-slate-400 font-semibold mb-1 block">วันที่ใบวางบิล From – To</label>
            <div className="flex items-center gap-1">
              <input
                type="date"
                value={filterDateFrom}
                onChange={e => setFilterDateFrom(e.target.value)}
                className="w-1/2 bg-slate-950 border border-slate-800 rounded-xl px-2 py-1.5 text-[11px] text-white focus:outline-none focus:border-emerald-500"
              />
          ...
          {/* Due Date Range */}
          <div>
            <label className="text-[10px] text-slate-400 font-semibold mb-1 block">Due Date From – To</label>
            <div className="flex items-center gap-1">
              <input
                type="date"
          ...
          {/* Sales Order / Invoice */}
          <div>
            <label className="text-[10px] text-slate-400 font-semibold mb-1 block">Sales Order / Invoice Ref</label>
            <div className="flex items-center gap-1">
              <input
                type="text"
                placeholder="เลข SO"
                ...
                className="w-1/2 bg-slate-950 border border-slate-800 rounded-xl px-2 py-1.5 text-xs text-white placeholder-slate-600 focus:outline-none focus:border-emerald-500"
              />
              <input
                type="text"
                placeholder="เลข INV"
          ...
          <div>
            <label className="text-[10px] text-slate-400 font-semibold mb-1 block">Payment Method & Action</label>
            <div className="flex items-center gap-2">
              <select
                value={filterPaymentMethod}
                onChange={e => setFilterPaymentMethod(e.target.value)}
                className="w-1/2 bg-slate-950 border border-slate-800 rounded-xl px-2 py-1.5 text-xs text-white focus:outline-none focus:border-emerald-500 cursor-pointer"
              >
              ...
              <button
                onClick={handleResetFilters}
                className="w-1/2 px-3 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-xl text-xs font-semibold border border-slate-700 transition cursor-pointer text-center"
              >
                ล้างตัวกรอง
              </button>
            </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="py-3.5 px-3 max-w-[200px]">
                        <div className="font-bold text-white truncate">{bl.customer_name}</div>
                        {bl.customer_tax_id && (
                          <div className="text-[10px] text-slate-500 font-mono">Tax: {bl.customer_tax_id}</div>
                        )}
                      </td>

                      {/* Reference Invoice */}
                      <td className="py-3.5 px-3 font-mono text-slate-300 max-w-[140px] truncate" title={invoiceNumbers}>
                        {invoiceNumbers}
                      </td>

                      {/* Reference SO */}
                      <td className="py-3.5 px-3 font-mono text-indigo-400 max-w-[120px] truncate" title={soNumbers}>
                        {soNumbers}
                      </td>
                      ...
                      {/* Paid Amount */}
                      <td className="py-3.5 px-3 text-right font-mono text-blue-400 whitespace-nowrap">
                      ...
                      <td className="py-3.5 px-3 text-center whitespace-nowrap">
                        <div className="flex items-center justify-center gap-1">
                          ...
                          <button
                            onClick={() => setViewingBilling(bl)}
                            className="flex items-center gap-1 px-2 py-1 bg-emerald-950/70 hover:bg-emerald-900/80 text-emerald-400 border border-emerald-800/80 rounded text-[10.5px] font-semibold transition cursor-pointer"
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-3xl max-h-[92vh] flex flex-col shadow-2xl overflow-hidden my-auto">
            ...
            <form onSubmit={handleSubmit} className="p-5 overflow-y-auto space-y-4 text-xs">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div>
                  <label className="text-[10px] text-slate-400 font-semibold mb-1 block">เลขที่ใบวางบิล (Billing Note No.)</label>
                  <input
                    type="text"
                    required
                    value={formBillingNo}
                    onChange={e => setFormBillingNo(e.target.value)}
                    className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-xs font-mono text-emerald-400 font-bold focus:outline-none focus:border-emerald-500"
                  />
                </div>
                <div>
                  <label className="text-[10px] text-slate-400 font-semibold mb-1 block">วันที่ใบวางบิล (Date)</label>
                  <input
                    type="date"
                    required
                ...
                <div>
                  <label className="text-[10px] text-slate-400 font-semibold mb-1 block">กำหนดชำระ (Due Date)</label>
                  <input
                    type="date"
                    required
                    value={formDueDate}
                    onChange={e => setFormDueDate(e.target.value)}
                    className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-xs text-amber-400 font-bold focus:outline-none focus:border-emerald-500"
                  />
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  <label className="text-[10px] text-slate-400 font-semibold mb-1 block">เลือกลูกค้าจากฐานข้อมูล</label>
                  <select
                ...
                <div>
                  <label className="text-[10px] text-slate-400 font-semibold mb-1 block">ชื่อลูกค้า (Company Name)</label>
                  <input
                    type="text"
                    required
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div className="sm:col-span-2">
                  <label className="text-[10px] text-slate-400 font-semibold mb-1 block">ที่อยู่ลูกค้า</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="text-[10px] text-slate-400 font-semibold mb-1 block">เลขประจำตัวผู้เสียภาษี</label>
                  <input
                    type="text"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
                <div>
                  <label className="text-[10px] text-slate-400 font-semibold mb-1 block">สถานะ</label>
                  <select
                    value={formStatus}
                ...
                <div>
                  <label className="text-[10px] text-slate-400 font-semibold mb-1 block">พนักงานขาย (Salesperson)</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="text-[10px] text-slate-400 font-semibold mb-1 block">ผู้นำส่งเอกสาร (Delivered By)</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="text-[10px] text-slate-400 font-semibold mb-1 block">วิธีชำระเงินที่แนะนำ</label>
                  <select
              ...
              <div className="border border-slate-800 rounded-xl p-3.5 bg-slate-950/60 space-y-3">
                <div className="flex flex-wrap items-center justify-between gap-2 pb-2 border-b border-slate-800/80">
                  <div>
                    <div className="flex items-center gap-2">
                      <span className="text-xs font-bold text-white">รายการใบแจ้งหนี้ / รายละเอียดที่เรียกเก็บ (Invoice Items)</span>
                  ...
                  <div className="flex items-center gap-1.5">
                    {formItems.length < MAX_ITEMS ? (
                      <>
                        ...
                        <button
                          ...
                          className="px-2 py-1 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-lg text-[11px] font-medium transition cursor-pointer"
                      ...
                      <button
                        ...
                        className="px-2.5 py-1 bg-amber-500/15 hover:bg-amber-500/25 border border-amber-500/30 text-amber-300 rounded-lg text-[11px] font-medium flex items-center gap-1 cursor-pointer transition"
                    ...
                    {formItems.length > 1 && (
                      <button
                        ...
                        className="px-2 py-1 bg-slate-800/80 hover:bg-slate-700 text-slate-400 rounded-lg text-[11px] transition cursor-pointer"
                ...
                <div className="space-y-2.5 max-h-[380px] overflow-y-auto pr-1">
                  {formItems.map((it, idx) => (
                    <div key={idx} className="bg-slate-900/90 p-3 rounded-xl border border-slate-800 hover:border-slate-700 transition space-y-2">
                      <div className="flex items-center justify-between gap-2 border-b border-slate-800/60 pb-1.5">
                        <div className="flex items-center gap-2">
                          <span className="font-mono text-emerald-400 font-bold text-xs bg-slate-950 px-2 py-0.5 rounded border border-slate-800">
                            ลำดับที่ #{idx + 1}
                          </span>
                          <span className="text-[11px] text-slate-400">รายการสินค้า/บริการ</span>
                        </div>
                        <div className="flex items-center gap-2">
                          {formItems.length > 1 && (
                            <button
                              ...
                              className="px-2 py-0.5 text-xs text-rose-400 hover:text-rose-300 hover:bg-rose-500/10 rounded transition cursor-pointer flex items-center gap-1"
                              title="ลบรายการนี้"
                            >
                              <Trash2 className="w-3.5 h-3.5" />
                              <span>ลบ</span>
                      ...
                      <div>
                        <input
                          ...
                          className="w-full bg-slate-950 border border-slate-800 rounded-lg px-3 py-1.5 text-xs text-slate-100 placeholder:text-slate-500 focus:border-emerald-500 focus:outline-none"
                        />
                      </div>
                      ...
                      <div className="grid grid-cols-12 gap-2 items-center">
                        <div className="col-span-2">
                          <label className="text-[9px] text-slate-400 font-semibold block mb-0.5">Quantity (จำนวน)</label>
                          <input
                            type="number"
                            min="0"
                            step="any"
                            ...
                            className="w-full bg-slate-950 border border-slate-800 rounded-lg px-2 py-1 text-xs text-white font-mono text-center focus:border-emerald-500 focus:outline-none"
                          />
                        </div>

                        <div className="col-span-2">
                          <label className="text-[9px] text-slate-400 font-semibold block mb-0.5">Unit (หน่วย)</label>
                          <input
                            type="text"
                            ...
                            className="w-full bg-slate-950 border border-slate-800 rounded-lg px-2 py-1 text-xs text-slate-200 text-center focus:border-emerald-500 focus:outline-none"
                          />
                        </div>

                        <div className="col-span-2">
                          <label className="text-[9px] text-slate-400 font-semibold block mb-0.5">Unit Price (ราคา/หน่วย)</label>
                          <input
                            type="number"
                            min="0"
                            step="any"
                            ...
                            className="w-full bg-slate-950 border border-slate-800 rounded-lg px-2 py-1 text-xs text-amber-400 font-mono text-right focus:border-emerald-500 focus:outline-none"
                          />
                        </div>

                        <div className="col-span-3">
                          <label className="text-[9px] text-slate-400 font-semibold block mb-0.5">Amount (จำนวนเงิน)</label>
                          <div className="relative">
                            ...
                            <input
                              ...
                              className="w-full bg-slate-950 border border-slate-800 rounded-lg pl-6 pr-2.5 py-1 text-xs text-emerald-400 font-mono font-bold text-right focus:border-emerald-500 focus:outline-none"
                            />
                          </div>
                        </div>

                        <div className="col-span-3">
                          <label className="text-[9px] text-slate-400 font-semibold block mb-0.5">Invoice / Ref No.</label>
                          <input
                            type="text"
                            ...
                            className="w-full bg-slate-950 border border-slate-800 rounded-lg px-2.5 py-1 text-xs text-white font-mono focus:border-emerald-500 focus:outline-none"
                          />
                        </div>
                      </div>
                    </div>
                ...
                <div className="pt-3 border-t border-slate-800 space-y-2">
                  <div className="flex flex-wrap items-center justify-between gap-2 text-xs">
                    ...
                    <div className="flex items-center gap-4 text-right">
                      <div>
                        <span className="text-[11px] text-slate-400 block">ยอดรวมก่อนภาษี (Subtotal)</span>
                        <span className="font-mono font-bold text-slate-200">
                          ฿{formSubtotal.toLocaleString('th-TH', { minimumFractionDigits: 2 })}
                        </span>
                      </div>
                      <div className="pl-4 border-l border-slate-800">
                        <span className="text-[11px] text-emerald-400 font-bold block">ยอดสุทธิรวมทั้งสิ้น (Total Calculated)</span>
                  ...
                  <div className="bg-emerald-950/30 border border-emerald-800/40 rounded-lg px-3 py-1.5 flex items-center justify-between text-xs">
                    <span className="text-emerald-400/80 font-medium">จำนวนเงินตัวอักษร (Thai Baht):</span>
                    <span className="font-semibold text-emerald-300 font-sans">{formThaiBahtText}</span>
                  </div>
                </div>
              </div>
              ...
              {/* Notes */}
              <div>
                <label className="text-[10px] text-slate-400 font-semibold mb-1 block">หมายเหตุ / คำชี้แจงในการวางบิล</label>
                <textarea
                  rows={2}
                  value={formNotes}
            ...
            <div className="p-5 space-y-4 text-xs">
              <div className="bg-slate-950 p-3 rounded-xl border border-slate-800 space-y-1">
                <div className="flex justify-between">
                  ...
                  <span className="font-mono font-bold text-emerald-400">{paymentModalBilling.billing_no}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-slate-400">ลูกค้า:</span>
                  <span className="font-bold text-white">{paymentModalBilling.customer_name}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-slate-400">ยอดวางบิลรวม:</span>
                  <span className="font-mono text-white">฿{paymentModalBilling.total_amount.toLocaleString()}</span>
                </div>
                <div className="flex justify-between border-t border-slate-850 pt-1 mt-1">
                  ...
                  <span className="font-mono font-bold text-amber-400">฿{(paymentModalBilling.outstanding_amount || paymentModalBilling.total_amount).toLocaleString()}</span>
                </div>
              </div>

              <div>
                <label className="text-[10px] text-slate-400 font-semibold mb-1 block">ยอดที่รับชำระ (บาท)</label>
                <input
                  type="number"
                  value={paymentAmount}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-[10px] text-slate-400 font-semibold mb-1 block">วิธีชำระเงิน</label>
                  <select
                    value={paymentMethod}
                ...
                <div>
                  <label className="text-[10px] text-slate-400 font-semibold mb-1 block">วันที่รับเงิน</label>
                  <input
                    type="date"
                    value={paymentDate}
              ...
              <div>
                <label className="text-[10px] text-slate-400 font-semibold mb-1 block">เลขอ้างอิงสลิป / เช็ค (Ref No.)</label>
                <input
                  type="text"
              ...
              <div>
                <label className="text-[10px] text-slate-400 font-semibold mb-1 block">หมายเหตุเพิ่มเติม</label>
                <input
                  type="text"
