...
export default function AdministrationView({ 
  ...
  const reloadUsers = async () => {
    ...
    try {
      const usersData = await CRMService.fetchUsers();
      if (usersData && usersData.length > 0) {
        const mapped: CRMUserSim[] = usersData.map((u: any) => ({
          id: u.id,
          name: u.fullname || u.username,
          email: u.email,
          role: (u.role === 'Admin' ? 'Admin' : u.role === 'Sales Manager' ? 'Sales Manager' : u.role === 'Auditor' ? 'System Administrator' : 'Sales') as UserRole,
          department: u.department || 'ฝ่ายงานขาย',
          status: u.status === 'Suspended' ? 'Inactive' : 'Active'
        }));
        setSimUsers(mapped);
      } else {
        setSimUsers(INITIAL_MOCK_USERS);
      }
    } catch (e) {
      console.warn("Failed to load users from Supabase, using cache or mock", e);
      setSimUsers(INITIAL_MOCK_USERS);
    } finally {
  ...
  const handleSaveUser = async (e: React.FormEvent) => {
    ...
    try {
      if (editingUserId) {
        const dbRole = userRole === 'System Administrator' ? 'Admin' : userRole;
        await CRMService.updateUser(editingUserId, {
          fullname: userName,
          email: userEmail,
          role: dbRole,
          status: userStatus === 'Inactive' ? 'Suspended' : 'Active'
        });
        ...
        const updated = simUsers.map(u => u.id === editingUserId ? {
          ...
          email: userEmail,
          role: userRole,
          department: userDept || u.department,
          status: userStatus
        } : u);
        setSimUsers(updated);
        ...
        const dbRole = userRole === 'System Administrator' ? 'Admin' : userRole;
        const newDbUser = await CRMService.addUser({
          fullname: userName,
        ...
        const newUser: CRMUserSim = {
          id: newDbUser.id || crypto.randomUUID(),
          name: userName,
          email: userEmail,
          role: userRole,
          department: userDept || 'ฝ่ายงานขาย',
          status: userStatus
        };
        setSimUsers([newUser, ...simUsers]);
        onToast(`ลงทะเบียนผู้ใช้ ${userName} สำเร็จแล้ว`, 'success');
      }
      setIsUserFormOpen(false);
    } catch (err: any) {
      console.error(err);
      onToast('เกิดข้อผิดพลาดในการบันทึกผู้ใช้: ' + (err.message || 'โปรดลองอีกครั้ง'), 'err');
    }
  };
  ...
  const handleDeleteUser = async (id: string, name: string) => {
    ...
    if (confirm(`คุณต้องการถอนรายชื่อผู้ใช้ "${name}" ยืนยันใช่หรือไม่?`)) {
      try {
        ...
        console.error(err);
        onToast('เกิดข้อผิดพลาดในการลบผู้ใช้งาน: ' + (err.message || 'โปรดลองอีกครั้ง'), 'err');
      }
    }
  };
