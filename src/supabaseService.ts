...
export const getSupabaseConfig = () => {
  ...
  const url = (customUrl || (import.meta as any).env.VITE_SUPABASE_URL || 'https://ctgjfgpovfvismgfutcl.supabase.co/rest/v1').trim();
  const key = (customKey || (import.meta as any).env.VITE_SUPABASE_ANON_KEY || 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImN0Z2pmZ3BvdmZ2aXNtZ2Z1dGNsIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODQ5NTQyODEsImV4cCI6MjEwMDUzMDI4MX0.LaQ7BXDg8Fo50MwxsKbpCgozWm2lyGz-JfsmRfgWJ4E').trim();
...
export const CRMService = {
  ...
  // -------------------------------------------------------------
  // USER SERVICES (public.users)
  // -------------------------------------------------------------
  async fetchUsers(): Promise<any[]> {
    const isCloud = await this.checkCloudConnection();
    if (isCloud && getConnectivityMode()) {
      try {
        const data = await apiFetch('/users?order=created_at.desc');
        if (Array.isArray(data)) {
          localStorage.setItem('crm_users_list', JSON.stringify(data));
          return data;
        }
      } catch (e) {
        console.warn('Failed to fetch users from Supabase, falling back to local cache', e);
      }
    }
  ...
  async addUser(user: any): Promise<any> {
    ...
    const newUser = {
      ...
      username: (user.username || `user_${Date.now()}`).trim().toLowerCase(),
      fullname: (user.fullname || '').trim(),
      email: (user.email || '').trim(),
      role: user.role || 'Sales Rep',
      status: user.status || 'Active',
      password: user.password || 'crm123456',
      created_at: now,
      updated_at: now
    };
    ...
    if (isCloud && getConnectivityMode()) {
      try {
        ...
        const created = (res && res[0]) ? res[0] : newUser;
        const currentList = JSON.parse(localStorage.getItem('crm_users_list') || '[]');
        currentList.unshift(created);
        localStorage.setItem('crm_users_list', JSON.stringify(currentList));
        return created;
      } catch (e) {
  ...
  async updateUser(id: string, updates: any): Promise<any> {
    ...
    if (isCloud && getConnectivityMode()) {
      try {
        const res = await apiFetch(`/users?id=eq.${id}`, {
          method: 'PATCH',
          headers: { 'Prefer': 'return=representation' },
          body: JSON.stringify(payload)
        });
        const updated = (res && res[0]) ? res[0] : payload;
        const currentList = JSON.parse(localStorage.getItem('crm_users_list') || '[]');
        const idx = currentList.findIndex((u: any) => u.id === id);
        if (idx !== -1) currentList[idx] = { ...currentList[idx], ...updated };
        localStorage.setItem('crm_users_list', JSON.stringify(currentList));
        return updated;
      } catch (e) {
      ...
      const currentList = JSON.parse(localStorage.getItem('crm_users_list') || '[]');
      const idx = currentList.findIndex((u: any) => u.id === id);
      if (idx !== -1) currentList[idx] = { ...currentList[idx], ...payload };
      localStorage.setItem('crm_users_list', JSON.stringify(currentList));
  ...
  async deleteUser(id: string): Promise<boolean> {
    ...
    if (isCloud && getConnectivityMode()) {
      try {
        await apiFetch(`/users?id=eq.${id}`, { method: 'DELETE' });
      } catch (e) {
    ...
    const currentList = JSON.parse(localStorage.getItem('crm_users_list') || '[]');
    const filtered = currentList.filter((u: any) => u.id !== id);
  ...
  async insertQuotation(quote: Omit<Quotation, 'id' | 'quotation_no' | 'created_at'>): Promise<Quotation> {
    const list = LocalDB.getQuotations();
    const qDate = quote.quotation_date || quote.issue_date || new Date().toISOString().slice(0, 10);
    let yr = "";
    let mm = "";
    if (qDate && typeof qDate === 'string' && qDate.includes("-")) {
    ...
    if (isCloud && getConnectivityMode()) {
      try {
        const allowedCols = [
          ...
          'quotation_date', 'validity_days', 'payment_term', 'status', 'sales_person_id',
          'total_value', 'tax_rate', 'grand_total', 'terms_conditions',
          'remarks', 'created_at', 'updated_at'
        ];
        const meta = {
          _so_meta: true,
          remarks: prepared.remarks || '',
          project_name: (prepared as any).project_name || prepared.title || '',
          job_no: (prepared as any).job_no || '',
          po_no: (prepared as any).po_no || '',
          delivery_plan: (prepared as any).delivery_plan || '',
          version: (prepared as any).version || '',
          customer_phone: prepared.customer_phone || '',
          customer_email: prepared.customer_email || '',
          attention: prepared.attention || '',
          cc: prepared.cc || '',
          currency: (prepared as any).currency || 'THB',
          exchange_rate: (prepared as any).exchange_rate || 1,
          items: prepared.items || [],
          sales_person: prepared.sales_person || (prepared as any).salesperson_name || ''
        };
        const dbPayload: any = {
          id: prepared.id,
          quotation_no: prepared.quotation_no,
          customer_id: prepared.customer_id || null,
          opportunity_id: prepared.opportunity_id || null,
          title: prepared.title || (prepared as any).subject || (prepared as any).project_name || 'Quotation',
          quotation_date: prepared.quotation_date || prepared.issue_date || new Date().toISOString().slice(0, 10),
          validity_days: prepared.validity_days || 30,
          payment_term: prepared.payment_term || '30 Days',
          status: prepared.status || 'Draft',
          salesperson_name: (prepared as any).salesperson_name || prepared.sales_person || '',
          total_value: prepared.total_value !== undefined ? prepared.total_value : (prepared.total_amount || 0),
          tax_rate: (prepared as any).tax_rate !== undefined ? (prepared as any).tax_rate : 7,
          grand_total: prepared.grand_total !== undefined ? prepared.grand_total : 0,
          terms_conditions: prepared.terms_conditions || '',
        ...
        for (const col of allowedCols) {
          if (dbPayload[col] !== undefined) {
            sanitized[col] = dbPayload[col];
          }
        }
        await apiFetch('/quotations', { method: 'POST', body: JSON.stringify(sanitized) });
      } catch (err: any) {
  ...
  async updateQuotation(id: string, updates: Partial<Quotation>): Promise<Quotation> {
    ...
    if (isCloud && getConnectivityMode()) {
      try {
        const allowedCols = [
          'quotation_no', 'customer_id', 'opportunity_id', 'title',
          'quotation_date', 'validity_days', 'payment_term', 'status', 'sales_person_id',
          'total_value', 'tax_rate', 'grand_total', 'terms_conditions',
          'remarks', 'updated_at'
        ];
        const meta = {
          ...
          project_name: (found as any).project_name || found.title || '',
          job_no: (found as any).job_no || '',
          po_no: (found as any).po_no || '',
          delivery_plan: (found as any).delivery_plan || '',
          version: (found as any).version || '',
          customer_phone: found.customer_phone || '',
          customer_email: found.customer_email || '',
          attention: found.attention || '',
          cc: found.cc || '',
          currency: (found as any).currency || 'THB',
          exchange_rate: (found as any).exchange_rate || 1,
          items: found.items || [],
          sales_person: found.sales_person || (found as any).salesperson_name || ''
        };
        const dbPayload: any = {
          quotation_no: found.quotation_no,
          customer_id: found.customer_id || null,
          opportunity_id: found.opportunity_id || null,
          title: found.title || (found as any).subject || (found as any).project_name || 'Quotation',
          quotation_date: found.quotation_date || found.issue_date || new Date().toISOString().slice(0, 10),
          validity_days: found.validity_days || 30,
          payment_term: found.payment_term || '30 Days',
          status: found.status || 'Draft',
          salesperson_name: (found as any).salesperson_name || found.sales_person || '',
          total_value: found.total_value !== undefined ? found.total_value : (found.total_amount || 0),
          tax_rate: (found as any).tax_rate !== undefined ? (found as any).tax_rate : 7,
          grand_total: found.grand_total !== undefined ? found.grand_total : 0,
          terms_conditions: found.terms_conditions || '',
          remarks: JSON.stringify(meta),
          updated_at: new Date().toISOString()
        };
        const sanitized: any = {};
        ...
        await apiFetch(`/quotations?id=eq.${id}`, { method: 'PATCH', body: JSON.stringify(sanitized) });
      } catch (err: any) {
